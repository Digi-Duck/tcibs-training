...
class ErrorController extends Controller
{
    ...
    public function token(){ //無效的token 所有需要登入的api
        return response()->json([
            "success" => false,
            "message" => "MSG_INVALID_ACCESS_TOKEN"
        ], 409);
    }
    public function password(){ //密碼長度不正確 2
        return response()->json([
            "success" => false,
            "message" => "MSG_PASSWORD_NOT_SECURE"
        ], 409);
    }
    public function log(){ //使用者不存在 帳密有誤 1
        return response()->json([
            "success" => false,
    ...
    public function post(){ //不存在的圖片 8 10 11 12
        return response()->json([
            "success" => false,
            "message" => "MSG_POST_NOT_ESISTS"
        ], 404);
    }
    public function comment(){ //不存在留言 或留言不屬於此貼文 13
        return response()->json([
            "success" => false,
            "message" => "MSG_COMMENT_NOT_ESISTS"
        ], 404);
    }
    public function user(){ //不存在的使用者 6
        return response()->json([
            "success" => false,
            "message" => "MSG_USER_NOT_ESISTS"
        ], 404);
    }
